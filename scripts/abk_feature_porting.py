...
def patch_close_range_hotpath(common_root: Path) -> dict[str, object]:
    ...
    if pick_old in text:
        ...
        if "if (!fd_is_open(fd, fdt))" not in pick_scope:
            # 6.1 hoisted the fdtable lookup and the lock into the caller and
            # returns NULL on miss; 5.15 takes files->file_lock itself, looks the
            # table up inside, and returns ERR_PTR. Insert the bitmap early-out
            # in whichever shape this tree uses, keeping its error convention.
            guard_6_1 = "\tif (fd >= fdt->max_fds)\n\t\treturn NULL;\n"
            guard_5_15 = "\tif (fd >= fdt->max_fds) {\n\t\tfile = ERR_PTR(-EINVAL);\n\t\tgoto out_unlock;\n\t}\n"
            if guard_6_1 in pick_scope:
                pick_scope = replace_once(
                    pick_scope,
                    guard_6_1,
                    guard_6_1 + "\tif (!fd_is_open(fd, fdt))\n\t\treturn NULL;\n",
        ...
        # Only the caller-locked (6.1) form uses these: abk_pick_file_for_close()
        # asserts files->file_lock, which the 5.15 __range_close no longer holds
        # because pick_file() takes it itself. Adding them there would be dead
        # code with a false assertion.
        caller_locked_pick = "\tif (fd >= fdt->max_fds)\n\t\treturn NULL;\n" in text
        if caller_locked_pick and "static inline unsigned int abk_close_range_limit(" not in text:
    ...
    # 6.1 lets __range_close own the lock, so the graft can hold it across the
    # bitmap walk and call the lockless pick_file() directly. 5.15's pick_file()
    # takes the lock itself and returns ERR_PTR, so the 5.15 form keeps calling
    # it unlocked once per fd -- the bitmap walk is still the win, skipping the
    # closed fds the old `while (fd <= max_fd)` loop visited one by one -- and
    # reads open_fds under rcu_read_lock() as __range_cloexec() does there.
    range_old_5_15 = """static inline void __range_close(struct files_struct *cur_fds, unsigned int fd,\n\t\t\t\t unsigned int max_fd)\n{\n\twhile (fd <= max_fd) {\n\t\tstruct file *file;\n\n\t\tfile = pick_file(cur_fds, fd++);\n\t\tif (!IS_ERR(file)) {\n\t\t\t/* found a valid file to close */\n\t\t\tfilp_close(file, cur_fds);\n\t\t\tcond_resched();\n\t\t\tcontinue;\n\t\t}\n\n\t\t/* beyond the last fd in that table */\n\t\tif (PTR_ERR(file) == -EINVAL)\n\t\t\treturn;\n\t}\n}\n"""
    range_new_5_15 = """/* ABK feature_porting: close_range() bitmap hotpath graft. */\nstatic inline void __range_close(struct files_struct *cur_fds, unsigned int fd,\n\t\t\t\t unsigned int max_fd)\n{\n\tstruct fdtable *fdt;\n\tunsigned int n;\n\n\trcu_read_lock();\n\tfdt = files_fdtable(cur_fds);\n\tn = last_fd(fdt);\n\tmax_fd = min(max_fd, n);\n\tfd = find_next_bit(fdt->open_fds, max_fd + 1, fd);\n\trcu_read_unlock();\n\n\twhile (fd <= max_fd) {\n\t\tstruct file *file;\n\n\t\tfile = pick_file(cur_fds, fd);\n\t\tif (!IS_ERR(file)) {\n\t\t\t/* found a valid file to close */\n\t\t\tfilp_close(file, cur_fds);\n\t\t\tcond_resched();\n\t\t} else if (PTR_ERR(file) == -EINVAL) {\n\t\t\t/* beyond the last fd in that table */\n\t\t\treturn;\n\t\t}\n\n\t\trcu_read_lock();\n\t\tfdt = files_fdtable(cur_fds);\n\t\tmax_fd = min(max_fd, last_fd(fdt));\n\t\tfd = find_next_bit(fdt->open_fds, max_fd + 1, fd + 1);\n\t\trcu_read_unlock();\n\t}\n}\n"""
    if range_old in text:
        text = replace_once(text, range_old, range_new, "feature_porting/close_range_range_close")
    else:
        text = replace_once(text, range_old_5_15, range_new_5_15, "feature_porting/close_range_range_close")
...
def patch_avg_idle_preemption_mode(common_root: Path) -> dict[str, object]:
    ...
    #
    # 6.1 already had SIS_UTIL for that, reading a precomputed nr_idle_scan out
    # of sd_llc_shared, so there the graft just deletes the SIS_PROP arms and
    # keeps the SIS_UTIL one. 5.15 has no SIS_UTIL and no nr_idle_scan (checked
    # against SUBLEVEL 167: both appear zero times), and SIS_PROP is the only
    # SCHED_FEAT in features.h, so deleting its arms outright would remove the
    # cap entirely and make every wakeup scan the whole LLC.
    #
    # So 5.15 keeps the SIS_PROP feature gate and its scan-cost accounting --
    # sd->avg_scan_cost is per-domain and costs nothing extra to maintain -- and
    # only drops the rq->wake_avg_idle prediction, budgeting from rq->avg_idle
    # directly. Same outcome for the field this graft exists to retire, without
    # uncapping the scan.
    select_old_5_15 = """static int select_idle_cpu(struct task_struct *p, struct sched_domain *sd, bool has_idle_core, int target)\n{\n\tstruct cpumask *cpus = this_cpu_cpumask_var_ptr(select_idle_mask);\n\tint i, cpu, idle_cpu = -1, nr = INT_MAX;\n\tstruct rq *this_rq = this_rq();\n\tint this = smp_processor_id();\n\tstruct sched_domain *this_sd;\n\tu64 time = 0;\n\n\tthis_sd = rcu_dereference(*this_cpu_ptr(&sd_llc));\n\tif (!this_sd)\n\t\treturn -1;\n\n\tcpumask_and(cpus, sched_domain_span(sd), p->cpus_ptr);\n\n\tif (sched_feat(SIS_PROP) && !has_idle_core) {\n\t\tu64 avg_cost, avg_idle, span_avg;\n\t\tunsigned long now = jiffies;\n\n\t\t/*\n\t\t * If we're busy, the assumption that the last idle period\n\t\t * predicts the future is flawed; age away the remaining\n\t\t * predicted idle time.\n\t\t */\n\t\tif (unlikely(this_rq->wake_stamp < now)) {\n\t\t\twhile (this_rq->wake_stamp < now && this_rq->wake_avg_idle) {\n\t\t\t\tthis_rq->wake_stamp++;\n\t\t\t\tthis_rq->wake_avg_idle >>= 1;\n\t\t\t}\n\t\t}\n\n\t\tavg_idle = this_rq->wake_avg_idle;\n\t\tavg_cost = this_sd->avg_scan_cost + 1;\n\n\t\tspan_avg = sd->span_weight * avg_idle;\n\t\tif (span_avg > 4*avg_cost)\n\t\t\tnr = div_u64(span_avg, avg_cost);\n\t\telse\n\t\t\tnr = 4;\n\n\t\ttime = cpu_clock(this);\n\t}\n"""
    select_new_5_15 = """/*\n * ABK feature_porting: avg_idle preemption mode simplification.\n * This tree has no SIS_UTIL, so keep the SIS_PROP scan budget but drop the\n * rq->wake_avg_idle prediction and derive it from rq->avg_idle directly.\n */\nstatic int select_idle_cpu(struct task_struct *p, struct sched_domain *sd, bool has_idle_core, int target)\n{\n\tstruct cpumask *cpus = this_cpu_cpumask_var_ptr(select_idle_mask);\n\tint i, cpu, idle_cpu = -1, nr = INT_MAX;\n\tstruct rq *this_rq = this_rq();\n\tint this = smp_processor_id();\n\tstruct sched_domain *this_sd;\n\tu64 time = 0;\n\n\tthis_sd = rcu_dereference(*this_cpu_ptr(&sd_llc));\n\tif (!this_sd)\n\t\treturn -1;\n\n\tcpumask_and(cpus, sched_domain_span(sd), p->cpus_ptr);\n\n\tif (sched_feat(SIS_PROP) && !has_idle_core) {\n\t\tu64 avg_cost, avg_idle, span_avg;\n\n\t\tavg_idle = this_rq->avg_idle / 2;\n\t\tavg_cost = this_sd->avg_scan_cost + 1;\n\n\t\tspan_avg = sd->span_weight * avg_idle;\n\t\tif (span_avg > 4*avg_cost)\n\t\t\tnr = div_u64(span_avg, avg_cost);\n\t\telse\n\t\t\tnr = 4;\n\n\t\ttime = cpu_clock(this);\n\t}\n"""
...
def patch_hugepage_fault_alloc_fastpath(common_root: Path) -> dict[str, object]:
    ...
    if huge_marker not in huge_text:
        ...
        huge_text = replace_once(
            huge_text,
            helper_anchor,
            helper_block if "struct folio" in huge_text else helper_block_5_15,
    ...
    # 5.15 form: same decomposition, page-typed. Note it keeps the page argument
    # (there is no folio to pass) and routes through the *_page helpers above.
    do_huge_old_5_15 = """static vm_fault_t __do_huge_pmd_anonymous_page(struct vm_fault *vmf,\n\t\t\tstruct page *page, gfp_t gfp)\n{\n\tstruct vm_area_struct *vma = vmf->vma;\n\tpgtable_t pgtable;\n\tunsigned long haddr = vmf->address & HPAGE_PMD_MASK;\n\tvm_fault_t ret = 0;\n\n\tVM_BUG_ON_PAGE(!PageCompound(page), page);\n\n\tif (mem_cgroup_charge(page, vma->vm_mm, gfp)) {\n\t\tput_page(page);\n\t\tcount_vm_event(THP_FAULT_FALLBACK);\n\t\tcount_vm_event(THP_FAULT_FALLBACK_CHARGE);\n\t\treturn VM_FAULT_FALLBACK;\n\t}\n\tcgroup_throttle_swaprate(page, gfp);\n\n\tpgtable = pte_alloc_one(vma->vm_mm);\n\tif (unlikely(!pgtable)) {\n\t\tret = VM_FAULT_OOM;\n\t\tgoto release;\n\t}\n\n\tclear_huge_page(page, vmf->address, HPAGE_PMD_NR);\n\t/*\n\t * The memory barrier inside __SetPageUptodate makes sure that\n\t * clear_huge_page writes become visible before the set_pmd_at()\n\t * write.\n\t */\n\t__SetPageUptodate(page);\n\n\tvmf->ptl = pmd_lock(vma->vm_mm, vmf->pmd);\n\tif (unlikely(!pmd_none(*vmf->pmd))) {\n\t\tgoto unlock_release;\n\t} else {\n\t\tpmd_t entry;\n\n\t\tret = check_stable_address_space(vma->vm_mm);\n\t\tif (ret)\n\t\t\tgoto unlock_release;\n\n\t\t/* Deliver the page fault to userland */\n\t\tif (userfaultfd_missing(vma)) {\n\t\t\tspin_unlock(vmf->ptl);\n\t\t\tput_page(page);\n\t\t\tpte_free(vma->vm_mm, pgtable);\n\t\t\tret = handle_userfault(vmf, VM_UFFD_MISSING);\n\t\t\tVM_BUG_ON(ret & VM_FAULT_FALLBACK);\n\t\t\treturn ret;\n\t\t}\n\n\t\tentry = mk_huge_pmd(page, vma->vm_page_prot);\n\t\tentry = maybe_pmd_mkwrite(pmd_mkdirty(entry), vma);\n\t\tpage_add_new_anon_rmap(page, vma, haddr, true);\n\t\tlru_cache_add_inactive_or_unevictable(page, vma);\n\t\tpgtable_trans_huge_deposit(vma->vm_mm, vmf->pmd, pgtable);\n\t\tset_pmd_at(vma->vm_mm, haddr, vmf->pmd, entry);\n\t\tupdate_mmu_cache_pmd(vma, vmf->address, vmf->pmd);\n\t\tadd_mm_counter(vma->vm_mm, MM_ANONPAGES, HPAGE_PMD_NR);\n\t\tmm_inc_nr_ptes(vma->vm_mm);\n\t\tspin_unlock(vmf->ptl);\n\t\tcount_vm_event(THP_FAULT_ALLOC);\n\t\tcount_memcg_event_mm(vma->vm_mm, THP_FAULT_ALLOC);\n\t}\n\n\treturn 0;\nunlock_release:\n\tspin_unlock(vmf->ptl);\nrelease:\n\tif (pgtable)\n\t\tpte_free(vma->vm_mm, pgtable);\n\tput_page(page);\n\treturn ret;\n\n}\n"""
    do_huge_new_5_15 = """static vm_fault_t __do_huge_pmd_anonymous_page(struct vm_fault *vmf,\n\t\t\tstruct page *page, gfp_t gfp)\n{\n\tstruct vm_area_struct *vma = vmf->vma;\n\tpgtable_t pgtable;\n\tunsigned long haddr = vmf->address & HPAGE_PMD_MASK;\n\tvm_fault_t ret;\n\n\tVM_BUG_ON_PAGE(!PageCompound(page), page);\n\n\tret = abk_thp_fault_charge_page(page, vma, gfp);\n\tif (ret)\n\t\treturn ret;\n\n\tpgtable = pte_alloc_one(vma->vm_mm);\n\tif (unlikely(!pgtable)) {\n\t\tret = VM_FAULT_OOM;\n\t\tgoto release;\n\t}\n\n\tabk_prep_anon_thp_page(page, vmf->address);\n\n\tvmf->ptl = pmd_lock(vma->vm_mm, vmf->pmd);\n\tif (unlikely(!pmd_none(*vmf->pmd)))\n\t\tgoto unlock_release;\n\n\tret = check_stable_address_space(vma->vm_mm);\n\tif (ret)\n\t\tgoto unlock_release;\n\n\t/* Deliver the page fault to userland */\n\tif (userfaultfd_missing(vma)) {\n\t\tspin_unlock(vmf->ptl);\n\t\tput_page(page);\n\t\tpte_free(vma->vm_mm, pgtable);\n\t\tret = handle_userfault(vmf, VM_UFFD_MISSING);\n\t\tVM_BUG_ON(ret & VM_FAULT_FALLBACK);\n\t\treturn ret;\n\t}\n\n\tabk_map_anon_page_pmd(page, pgtable, vmf->pmd, vma,\n\t\t\t      haddr, vmf->address);\n\tspin_unlock(vmf->ptl);\n\treturn 0;\n\nunlock_release:\n\tspin_unlock(vmf->ptl);\nrelease:\n\tif (pgtable)\n\t\tpte_free(vma->vm_mm, pgtable);\n\tput_page(page);\n\treturn ret;\n\n}\n"""
    if "static vm_fault_t __do_huge_pmd_anonymous_page(struct vm_fault *vmf,\n\t\t\tstruct folio *folio, gfp_t gfp)\n{" not in huge_text and "abk_map_anon_page_pmd(page, pgtable" not in huge_text:
        huge_text = replace_within(
            huge_text,
            "static vm_fault_t __do_huge_pmd_anonymous_page(struct vm_fault *vmf,\n",
            "\n/*\n * always: directly stall for all thp allocations\n",
            do_huge_old if do_huge_old in huge_text else do_huge_old_5_15,
            do_huge_new if do_huge_old in huge_text else do_huge_new_5_15,
    ...
    # 5.15's entry allocates with alloc_hugepage_vma() + prep_transhuge_page()
    # and hands __do_huge_pmd_anonymous_page() a page. Its khugepaged_enter()
    # also returns an error to propagate, which abk_thp_fault_prepare() folds in.
    fault_entry_new_5_15 = """vm_fault_t do_huge_pmd_anonymous_page(struct vm_fault *vmf)\n{\n\tstruct vm_area_struct *vma = vmf->vma;\n\tgfp_t gfp;\n\tstruct page *page;\n\tunsigned long haddr = vmf->address & HPAGE_PMD_MASK;\n\tvm_fault_t ret;\n\n\tret = abk_thp_fault_prepare(vmf, haddr);\n\tif (ret)\n\t\treturn ret;\n\n\tif (!(vmf->flags & FAULT_FLAG_WRITE) &&\n\t\t\t!mm_forbids_zeropage(vma->vm_mm) &&\n\t\t\ttransparent_hugepage_use_zero_page())\n\t\treturn abk_do_huge_pmd_anonymous_zero_page(vmf, haddr);\n\n\tpage = abk_thp_fault_alloc_page(vma, haddr, &gfp);\n\tif (unlikely(!page))\n\t\treturn abk_thp_fault_fallback(false);\n\treturn __do_huge_pmd_anonymous_page(vmf, page, gfp);\n}\n"""
    if "ret = abk_thp_fault_prepare(vmf, haddr);" not in huge_text:
        huge_text = replace_scope(
            huge_text,
            "vm_fault_t do_huge_pmd_anonymous_page(struct vm_fault *vmf)\n{",
            "\nstatic void insert_pfn_pmd(struct vm_area_struct *vma, unsigned long addr,\n",
            fault_entry_new if "struct folio *folio;" in huge_text else fault_entry_new_5_15,
    ...
    if memory_marker not in memory_text:
        ...
        # 5.15 has no per-VMA lock, so no FAULT_FLAG_VMA_LOCK arm to preserve.
        memory_old_5_15 = """static inline vm_fault_t create_huge_pmd(struct vm_fault *vmf)\n{\n\tif (vma_is_anonymous(vmf->vma))\n\t\treturn do_huge_pmd_anonymous_page(vmf);\n\tif (vmf->vma->vm_ops->huge_fault)\n\t\treturn vmf->vma->vm_ops->huge_fault(vmf, PE_SIZE_PMD);\n\treturn VM_FAULT_FALLBACK;\n}\n"""
        memory_new_5_15 = """/* ABK feature_porting: hugepage fault alloc fastpath routing helper. */\nstatic inline vm_fault_t abk_create_anonymous_huge_pmd(struct vm_fault *vmf)\n{\n\treturn do_huge_pmd_anonymous_page(vmf);\n}\n\nstatic inline vm_fault_t create_huge_pmd(struct vm_fault *vmf)\n{\n\tif (vma_is_anonymous(vmf->vma))\n\t\treturn abk_create_anonymous_huge_pmd(vmf);\n\tif (vmf->vma->vm_ops->huge_fault)\n\t\treturn vmf->vma->vm_ops->huge_fault(vmf, PE_SIZE_PMD);\n\treturn VM_FAULT_FALLBACK;\n}\n"""
        use_6_1 = memory_old in memory_text
        memory_text = replace_once(
            memory_text,
            memory_old if use_6_1 else memory_old_5_15,
            memory_new if use_6_1 else memory_new_5_15,
